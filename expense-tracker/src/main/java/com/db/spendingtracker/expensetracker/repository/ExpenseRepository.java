...
public interface ExpenseRepository extends JpaRepository<Expense,Long>,ExpenseRepositoryCustom {
	...
	@Query(value="SELECT sum(e.amount) from Expense e where e.user.username = ?#{principal.username} and MONTH(expenseDate) = MONTH(CURRENT_DATE())")
	public Double totalExpenses();
	
	@Query(value="select c.name as category, sum(e.amount) as amount from Category c, Expense e where c.id=e.category.id and c.user.username=e.user.username and e.user.username=?#{principal.username} and MONTH(e.expenseDate) = MONTH(CURRENT_DATE()) group by c.name")
	public List<ExpenseByCategory> getExpensesPerCategory();
	
	@Query(value="select e.paymentType as paymentType, sum(e.amount) as amount from Expense e where e.user.username = ?#{principal.username} and MONTH(expenseDate) = MONTH(CURRENT_DATE()) group by e.paymentType")
